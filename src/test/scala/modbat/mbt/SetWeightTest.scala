...
class SetWeightTest extends FlatSpec with Matchers {
  "SetWeight1" should "pass with one transition" in {
    val result = ModbatTestHarness.testMain(Array("-s=1", "-n=1", "--no-redirect-out", "--classpath=build/modbat-test.jar", "-s",  "modbat.test.SetWeight1"), ModbatTestHarness.setTestJar)
  ...
  "SetWeight2" should "pass with two transitions" in {
    val result = ModbatTestHarness.testMain(Array("-s=1", "-n=1", "--no-redirect-out", "--classpath=build/modbat-test.jar", "-s", "modbat.test.SetWeight2"), ModbatTestHarness.setTestJar)
    result._1 should be(0)
    result._2.filter(_.contains("1 tests executed, 1 ok, 0 failed.")) should not be empty
  ...
  "SetWeight3" should "pass with three transitions" in {
    val result = ModbatTestHarness.testMain(Array("-s=1", "-n=1", "--no-redirect-out", "--classpath=build/modbat-test.jar", "-s", "modbat.test.SetWeight3"), ModbatTestHarness.setTestJar)
    result._1 should be(0)
    result._2.filter(_.contains("1 tests executed, 1 ok, 0 failed.")) should not be empty
