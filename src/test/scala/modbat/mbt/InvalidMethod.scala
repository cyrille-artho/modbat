...
class InvalidMethod extends fixture.FlatSpec with fixture.TestDataFixture with Matchers {
  "InvalidMethod1" should "pass" in { td =>
    val result = ModbatTestHarness.testMain(Array("--mode=dot","modbat.test.InvalidMethod"), ModbatTestHarness.setTestJar, td)
  ...
  "InvalidMethod3" should "pass" in { td =>
    val result = ModbatTestHarness.testMain(Array("--mode=dot","--auto-labels","modbat.test.InvalidMethod"), ModbatTestHarness.setTestJar, td)
    result._1 should be(0)
    result._3 should not be empty
  }
